...
def load_quantiacs_futures(period: int):
	...
	V = arr.sel(field = 'vol').to_pandas().reset_index()
...
def data_split_single(df, start, end):
	"""Gets a single chunk of data between start and end"""
	data = df[(df['date'] >= start) & (df['date'] < end)]
	data.sort_values(['date', 'asset'], ignore_index=True)
...
def preprocess_data(quantiacs=True, period=252*50):
	"""
	...
	if quantiacs == True:
		df = load_quantiacs_futures(period=period)
	else:
	...
	#print("Dropping {} rows with Missing Values".format(missing_vals.max()))
	
	# drop rows with missing values, but drop ALL ROWS from that date!
	missing_macd = df_final.loc[df_final['macd'].isna(), 'date'].unique()
	df_final = df_final.drop(df_final[df_final['date'].isin(missing_macd)].index)

	missing_rsi = df_final.loc[df_final['rsi'].isna(), 'date'].unique()
	df_final = df_final.drop(df_final[df_final['date'].isin(missing_rsi)].index)

	missing_cci = df_final.loc[df_final['cci'].isna(), 'date'].unique()
	df_final = df_final.drop(df_final[df_final['date'].isin(missing_cci)].index)

	missing_adx = df_final.loc[df_final['adx'].isna(), 'date'].unique()
	df_final = df_final.drop(df_final[df_final['date'].isin(missing_adx)].index)
...
def calculate_turbulence(df):
	...
	for i in range(start, len(unique_date)):
		...
		hist_price = df_price_pivot[[np.datetime64(n) in unique_date[0:i] for n in df_price_pivot.index]]
